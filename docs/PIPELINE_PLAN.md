...
Этапы 1–4 — мой бэкенд. Версионирование цен и флаг аномалии >50% в
`_bulk_insert_items` (ТЗ 4.4/5) — **реализованы** (миграция 002,
`repository._save_item_versioned`); конвертация валют в KZT — `utils/currency.py`,
применяется в `ArchiveParser`. Импорт справочника и `/stats` (ТЗ 4.5/4.6) — готовы.
